...
Whiteboard использует [tldraw](https://tldraw.dev) — коммерческую библиотеку с платной лицензией.

- **Для локальной разработки** — ключ не нужен, оценочный режим работает полностью.
- **Для production** — если whiteboard используется активно, получите бесплатный **Hobby license** на [tldraw.dev/r/pricing](https://tldraw.dev/r/pricing) (для личных/некоммерческих проектов).
- **Как указать ключ:**
  1. Получите ключ на сайте tldraw
  2. Добавьте в `.env`:
     ```env
     TLDRAW_LICENSE_KEY=your_key_here
     ```
  3. Перезапустите frontend (`pnpm dev`) — ключ встроится автоматически.
...
- **Порты:**
  ...
  - `7882` — UDP (RTC, медиа)
- **Конфиг:** `livekit/livekit.prod.yaml` (production)
...
- **Модульная архитектура:**
  - `src/core/agent.py` — основной оркестратор
...
- Пересобрей: `docker-compose build --no-cache frontend && docker-compose up -d`
