...
| Параметр | Значение |
|---|---|
| Размер | 26.7 МБ (27 950 890 байт) |
| SHA-256 | `3f31fcd51638c62643dc49410c69ae8800672f581c9f8b1277437f3b2e8e0914` |
| ABI | `arm64-v8a` только |
| minSdk / targetSdk | 24 / 36 |
| Разрешения | INTERNET, ACCESS_NETWORK_STATE, RECORD_AUDIO, MODIFY_AUDIO_SETTINGS, CAMERA, POST_NOTIFICATIONS |
| `usesCleartextTraffic` | `true` |
| Подпись | Android Debug (`CN=Android Debug`) — ставится сайдлоадом |
| Путь | `dist-android/mutty-0.1.0-arm64-debug.apk` |

- **🚫 Блокер доставки — прав на запись нет вообще.** Оба пути упираются в токен:
  - `gh release create` → HTTP 403 «Resource not accessible by personal access token»
  - `git push origin share` → `Permission to Gaigen/Mutty.git denied to Virg29`, 403

  При этом `GET /repos/Gaigen/Mutty` возвращает `"push": true` — это роль аккаунта,
  а не область действия fine-grained PAT. Токену не выдан `Contents: Read and write`
  на этот репозиторий, поэтому API-ответ о правах вводит в заблуждение.

  Разблокируется одним из: расширить права PAT до `Contents: Read and write` для
  `Gaigen/Mutty`; выдать токен от владельца (Gaigen); либо забрать APK из воркспейса вручную.
- **Статус:** ✅ артефакт готов, ⏸ доставка и push заблокированы правами токена.
  Два коммита ждут отправки локально в `share`.

**Грабли со сборкой APK:** после смены размера нативной библиотеки Gradle обновляет
архив инкрементально и **не освобождает место** от прежней версии — APK остался
161 МБ при 27.8 МБ реальных данных (дыра ~148 МБ внутри zip). Лечится удалением
`gen/android/app/build` перед пересборкой. Стрип символов задаётся через
`CARGO_PROFILE_DEV_STRIP=symbols` (переменная действует только на один запуск и
не влияет на десктопную разработку); удаление `jniLibs.keepDebugSymbols` само по
себе не помогает — AGP пропускает стрип, когда NDK не сконфигурирован в Gradle.

### Этап 6 — Итерации

- **Статус:** ✅ **не потребовались.** DoD пройден с первой сборки.

## 🎉 DoD достигнут

Проверено на живом устройстве (Poco X5, Android 13): APK установился, приветственный
экран отработал, подключение к серверу прошло, **голосовая связь, камера и чат
работают**. Ни одной итерации отладки не понадобилось, `adb logcat` не пригодился.

Что оказалось решающим и неочевидным заранее:

1. **wry уже содержит мост разрешений** (`RustWebChromeClient.onPermissionRequest`) —
   ожидавшийся главный риск оказался нулевым, Kotlin править не пришлось. Но работает
   он только при объявленных в манифесте разрешениях, поэтому этап 2 был обязателен.
2. **`lib.rs` был настоящим блокером** — пустая заглушка означала бы приложение без
   плагинов и команд, падающее на первом `invoke`.
3. **Заглушки desktop-команд** сняли необходимость править фронтенд вообще.
