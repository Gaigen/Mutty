...
/**
 * Invisible handler component: applies audio settings to LiveKit track.
 * Lives inside <LiveKitRoom>, works continuously — regardless of settings panel being open.
 */
export default function AudioHandler() {
  ...
  // Ref — read saved deviceId without adding to deps (avoid feedback loop)
  ...
  // Ref needed to access current function from effect with [room]-deps
  const applySpeakerRef = useRef(applySpeakerDevice);
  applySpeakerRef.current = applySpeakerDevice;

  // Immediately on device change
  useEffect(() => {
    applySpeakerDevice();
  }, [applySpeakerDevice]);

  // With delay on room connect (AudioContext not ready instantly)
  useEffect(() => {
    if (!room) return;
  ...
  // Update noise gate params without restarting processor
  useEffect(() => {
    noiseGateRef.current.options = {
      threshold: audioSettings.noiseGateThreshold,
      attack: audioSettings.noiseGateAttack,
