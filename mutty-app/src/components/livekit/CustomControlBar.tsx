...
const CONTROL_DEFS: ControlDef[] = [
  ...
  { id: 'theaterMode',   priority: 2, estimatedWidth: 42 },
  { id: 'rightControls', priority: 3, estimatedWidth: 120 },
];

export function CustomControlBar({ controls, rightControls, theaterMode, onTheaterModeChange, style, ...props }: CustomControlBarProps) {
  ...
  return (
    ...
    >
      ...
      {/* Theater mode — visible in main bar */}
      {onTheaterModeChange && visibleIds.has('theaterMode') && (
        <div data-control-id="theaterMode">
          <button
            type="button"
            className="lk-button"
          ...
          >
            {theaterMode ? <Shrink size={18} aria-hidden /> : <Expand size={18} aria-hidden />}
          </button>
        </div>
      )}
      ...
      {showBurger && hasBurgerContent && (
        <BurgerMenu>
          ...
          {burgerIds.has('theaterMode') && onTheaterModeChange && (
            <BurgerMenuItem
              icon={theaterMode ? <Shrink size={16} /> : <Expand size={16} />}
              label={theaterMode ? 'Exit theater mode' : 'Theater mode'}
              onClick={() => onTheaterModeChange(!theaterMode)}
            />
          )}
          {(burgerIds.has('screenShare') || burgerIds.has('chat') || burgerIds.has('theaterMode')) && burgerIds.has('rightControls') && (
            <BurgerMenuDivider />
          )}
