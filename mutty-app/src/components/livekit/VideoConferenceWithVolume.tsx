...
export function VideoConferenceWithVolume({
  ...
  return (
    <div className={`lk-video-conference${theaterMode ? ' mutty-theater-mode' : ''}`} {...props}>
      {isWeb() && (
        <LayoutContextProvider value={layoutContext} onWidgetChange={widgetUpdate}>
          ...
          <div
            ...
            style={{ display: theaterMode ? 'none' : (widgetState.showChat ? 'block' : 'none') }}
          />
          <div
            className="chat-panel-wrapper"
            data-open={widgetState.showChat && !theaterMode}
            data-resizing={isResizing}
            style={{
              display: theaterMode ? 'none' : 'flex',
